...
namespace MyAnimeListTests
{
    ...
    public class Tests
    {
        ...
        public async Task Setup()
        {
            ...
            firefoxOptions.LogLevel = FirefoxDriverLogLevel.Trace; // Enable detailed logging
        ...
        public async Task UA01UserRegistration_ValidInput_SuccessfulRegistration()
        {
            ...
            {
                // Enter email
                IWebElement emailField = wait.Until(ExpectedConditions.ElementIsVisible(By.Id("loginEmail")));
                emailField.SendKeys(testEmail);

                // Enter username
                IWebElement usernameField = wait.Until(ExpectedConditions.ElementIsVisible(By.Name("user_name")));
                usernameField.SendKeys(testUserName);
                ...
                // Check if the confirmation password field is present
                var confirmPasswordFields = driver.FindElements(By.Id("password_confirm"));
                if (confirmPasswordFields.Count > 0)
                {
                    // If the confirmation password field is present, enter the password again
                    IWebElement confirmPasswordField = confirmPasswordFields[0];
                    confirmPasswordField.SendKeys(testPassword);
                }

                // Select birthday
                SelectElement monthSelect = new SelectElement(wait.Until(ExpectedConditions.ElementIsVisible(By.Name("birthday[month]"))));
                monthSelect.SelectByValue("1");  // January

                SelectElement daySelect = new SelectElement(wait.Until(ExpectedConditions.ElementIsVisible(By.Name("birthday[day]"))));
                daySelect.SelectByValue("1");    // 1st day

                SelectElement yearSelect = new SelectElement(wait.Until(ExpectedConditions.ElementIsVisible(By.Name("birthday[year]"))));
                yearSelect.SelectByValue("2000"); // Year 2000

                // 'Create Account' button
                IWebElement createAccountButton = wait.Until(ExpectedConditions.ElementToBeClickable(By.Id("create-account")));
                createAccountButton.Click();

                // Wait for the success message to appear
                IWebElement successMessage = wait.Until(ExpectedConditions.ElementIsVisible(By.Id("ga_goodresult")));

                // Verify that the success message is displayed
                ClassicAssert.IsTrue(successMessage.Displayed, "The registration was not successful.");
            }
            catch (WebDriverTimeoutException ex)
            {
                // Log detailed information to help diagnose the issue
                Console.WriteLine("Timeout occurred: " + ex.Message);
                Console.WriteLine("Page source: " + driver.PageSource); // Output page source for inspection
                throw;
            }
            catch (NoSuchElementException ex)
            {
                // Log detailed information if an element is not found
        ...
        public void LM05_ScoringAnime_VeryGood_ScoreUpdated()
        {
            ...
            ((IJavaScriptExecutor)driver).ExecuteScript("document.getElementById('myinfo_score').value='8';");

            // Trigger the change event (if necessary)
            ((IJavaScriptExecutor)driver).ExecuteScript("var element = document.getElementById('myinfo_score'); var event = new Event('change'); element.dispatchEvent(event);");

            // Retrieve the selected option's text using JavaScript
            string selectedOption = ((IJavaScriptExecutor)driver).ExecuteScript("return document.getElementById('myinfo_score').options[document.getElementById('myinfo_score').selectedIndex].text").ToString();

            // Assert that the correct option is selected
            ClassicAssert.AreEqual("(8) Very Good", selectedOption.Trim(), "The '(8) Very Good' option is not selected.");
        }
        ...
        public void LM07ExportingAnimeList_AnimeList_FileDownloaded()
        {
            ...
            {
                Console.WriteLine("Creating download directory: " + downloadPath);
        ...
        public void LM08ExportingMangaList_MangaList_FileDownloaded()
        {
            ...
            // Use the environment to get the correct downloads path
            string downloadPath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.UserProfile), "Downloads");
            ...
            ClassicAssert.IsNotNull(recentFile, "No file was downloaded.");
            ClassicAssert.IsTrue(recentFile.Contains("mangalist_"), $"The downloaded file '{recentFile}' does not start with 'mangalist_'.");
        }
